...
function recursiveProcessDirectory(path, previouslyProcessed) {
	...
	recursiveReaddir(path, (err, files) => {
		...
		// console.error("SHA1____________________________________ mtime________ Path__ ");

		var fileCount = files.length;

		var processFile = function(files, i) {
			if(files[i]) {
				var f = files[i];
				if(previouslyProcessed && fileProcessed(file, previouslyProcessed)) {
					...
					processedFiles ++;
					try {
						sha1File(f, function(shaSum) {
							// var mtime = fs.statSync(f).mtimeMs;
							// process.stdout.write( shaSum + " " + mtime + " " + f + "\n" );
